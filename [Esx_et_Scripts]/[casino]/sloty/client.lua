...
Citizen.CreateThread(function()
	while true do
		...
		local Dystans = GetDistanceBetweenCoords(PozycjaGracza, -1697.89, -1093.95, 14.15, true)
		local Dystans2 = GetDistanceBetweenCoords(PozycjaGracza, 964.38, 33.71, 74.88, true)
		...
		if Dystans <= 2.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu, "Appuyez sur [~g~E~s~] pour acheter des Jetons", 0.55, 1.5, "~b~Caisse", 0.7)
		...
		if Dystans2 <= 6.0 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu2, "Appuyez sur [~g~E~s~] pour parler au Barman", 0.55, 1.5, "~b~BAR", 0.7)
		...
		if Dystans3 <= 15 then
			...
			ESX.Game.Utils.DrawText3D(PozycjaTekstu3, "Appuyez sur [~g~E~s~] pour parler au Barman", 0.55, 1.5, "~b~BAR", 0.7)
...
function OtworzMenuKasyna()
	ESX.UI.Menu.Open(
      ...
      {
          title    = 'Casino de Panama  - Caisse',
          align    = 'left',
          elements = {
			{label = "Acheter des Jetons", value = "buy"},
			{label = "Echanger ces Jetons", value = "sell"},
		  }
      },
	  function(data, menu)
...
RegisterNUICallback('wygrana', function(data)
	TriggerEvent('pNotify:SendNotification', {text = 'Vous avez gagné '..data.win..' Jetons!'})
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Config.Sloty do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Appuyez sur ~INPUT_PICKUP~ pour jouer aux machines a sous.')
		...
		for i=1, #Config.Ruletka do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Appuyez sur ~INPUT_PICKUP~ pour jouer a la roulette.')
		...
		for i=1, #Config.Blackjack do
			...
			if dis <= 2.0 then
				ESX.ShowHelpNotification('Appuyez sur ~INPUT_PICKUP~ pour jouer au blackjack.')
...
local coordonate = {
    {1030.75, 59.67, 72.48, nil, 4.04, nil, 1535236204},
