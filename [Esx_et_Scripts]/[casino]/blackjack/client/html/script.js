...
$(function() {
    window.addEventListener('message', function(event) {
        if (event.data.type == "enableui") {
			...
			if (event.data.enable) {			
				...
				$hit.textContent = '[Nouvelle partie] - Multi x2';
				$stay.textContent = '[Mise +] +100';
				$score.textContent = '[Mise -] -100';
	...
	$hit.addEventListener('click', function () {
		...
		if ($hit.textContent.includes("[Nouvelle partie] - Multi x2") && BlackJack.bet >= 100) {
			$.post("http://blackjack/card", JSON.stringify({}));
			$('#ownedPoints').empty().append(0);
			...
			var prom = new Promise(function(res, rej) {
				...
				$score.textContent = "Le Croupier a " + BlackJack.getScore(BlackJack.dealerHand.slice(0, 1)) + " points.";
			});
			...
			var prom = new Promise(function(res, rej) {
				...
				if (BlackJack.didBust(BlackJack.hand)) {
					...
					BlackJack.Message('Vous avez perdu avec ' + BlackJack.getScore(BlackJack.hand) + ' points.');
					$hit.textContent = '[Nouvelle partie] - Multi x2';
					$stay.textContent = '[Mise +] +100';
					$score.textContent = '[Mise -] -100';
					$('#ownedPoints').empty().append(0);
	...
	$stay.addEventListener('click', function () {
		...
		if ($stay.textContent.includes("[Mise +] +100")) {
			...
			if (BlackJack.getScore(BlackJack.dealerHand) > BlackJack.getScore(BlackJack.hand)) {
				...
				$score.textContent = '[Mise -] -100';
				BlackJack.Lost();
				BlackJack.Message('Vous avez perdu avec ' + BlackJack.getScore(BlackJack.hand) + ' point contre ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedBet').empty().append(100);
				queActions(0);
				...
				$score.textContent = '[Mise -] -100';
				BlackJack.Win();
				BlackJack.Message('Vous avez gagné' + BlackJack.getScore(BlackJack.hand) + ' contre ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
				$('#ownedPoints').empty().append(0);
				$('#ownedBet').empty().append(100);
				queActions(0);
			} else {
				...
				setInterval(function(){ 
					...
					if (BlackJack.didBust(BlackJack.dealerHand)) {
						...
						$score.textContent = '[Mise -] -100';
						BlackJack.Win();
						BlackJack.Message('Vous avez gagnez avec ' + BlackJack.getScore(BlackJack.hand) + ' points contre ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
						$('#ownedPoints').empty().append(0);
						$('#ownedBet').empty().append(100);
					} else if (BlackJack.getScore(BlackJack.dealerHand) > BlackJack.getScore(BlackJack.hand)) {
						$.post("http://blackjack/card", JSON.stringify({}));
						...
						$score.textContent = '[Mise -] -100';
						BlackJack.Lost();
						BlackJack.Message('Vous perdez avec ' + BlackJack.getScore(BlackJack.hand) + ' contre ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
						$('#ownedPoints').empty().append(0);
						$('#ownedBet').empty().append(100);
					} else if (BlackJack.getScore(BlackJack.dealerHand) == BlackJack.getScore(BlackJack.hand)) {
						$.post("http://blackjack/card", JSON.stringify({}));
						...
						$score.textContent = '[Mise -] -100';
						BlackJack.Win();
						BlackJack.Message('Vous avez gagné avec ' + BlackJack.getScore(BlackJack.hand) + ' points contre ' + BlackJack.getScore(BlackJack.dealerHand) + ".");
						$('#ownedPoints').empty().append(0);
						$('#ownedBet').empty().append(100);
					}
					queActions(0)
	...
	$score.addEventListener('click', function () {
		...
		if ($score.textContent.includes("[Mise -] -100")) {
			$.post("http://blackjack/bet", JSON.stringify({}));
