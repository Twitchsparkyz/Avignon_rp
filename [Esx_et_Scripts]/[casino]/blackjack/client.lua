...
AddEventHandler('route68_blackjack:start', function()
	ESX.TriggerServerCallback('route68_blackjack:check_money', function(quantity)
		if quantity >= 100 then
			...
			ESX.ShowNotification("Vous avez besoin d'au moins 100 jetons pour jouer!")
		end
	end, '')
	--roulette_menu()
end)
...
RegisterNUICallback('escape2', function(data, cb)
	...
	TriggerEvent('pNotify:SendNotification', {text = "Vous n'avez plus de jetons!"})
end)
...
RegisterNUICallback('LostBet', function(data, cb)
	cb('ok')
	local count = data.bets
	TriggerEvent('pNotify:SendNotification', {text = "Vous avez perdu "..count.." Jetons!"})
end)
