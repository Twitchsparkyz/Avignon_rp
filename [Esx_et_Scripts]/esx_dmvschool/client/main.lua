...
local v1 = vector3(228.70, 374.61, 107.11)
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "T'es chaud pour le permis ?")
        end
    end
end)

Citizen.CreateThread(function()
    local hash = GetHashKey("s_m_o_busker_01")
    while not HasModelLoaded(hash) do
    RequestModel(hash)
    Wait(20)
    end
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_o_busker_01", 228.70, 374.61, 105.11, 157.31, false, true)
