			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
						...
						exports.JD_logs:discord("Braquage Bijouterie", GetPlayerServerId(PlayerId()), 0, '#FFFFFF', 'Bijouterie')
...
Citizen.CreateThread(function()
	-- TriggerEvent('lester:createBlip', 77, 884.54, -952.18, 44.11)
	while true do
							...
							if quantity >= Config.MaxJewelsSell then
								ESX.TriggerServerCallback('esx_vangelico_robbery:conteggio', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSell then
										...
										exports.JD_logs:discord("Revente Bijoux", GetPlayerServerId(PlayerId()), 0, '#FFFFFF', 'Bijouterie')
										blip = false
									else
										blip = false
