...
RegisterNetEvent('th:prendreotage')
AddEventHandler('th:prendreotage', function()
	takeHostage()
end)
...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Tu as besoin d\'un pistolet chargé pour prendre en otage !")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Personne ne peut être pris en otage!")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuyez [G] pour relâcher, [H] pour tuer")
