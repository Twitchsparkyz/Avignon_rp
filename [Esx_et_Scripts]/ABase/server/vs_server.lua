...
admins = {
    'steam:11000011137e8a8', -- Ethan
    'steam:1100001036abf35', -- Josh
    'steam:1100001141eb0cb', -- Lilian
    'steam:110000119821271', -- Ludo
    'steam:11000010ee4ea9f', -- Micky
    'steam:11000013c0d1cae', -- Paul SHELBY
}

-- Mettre sur "false" si vous ne souhaitez pas que la météo change automatiquement toutes les 20 minutes.
DynamicWeather = false

--------------------------------------------------
debugprint = false -- Ne touchez pas ceci à moins que vous ne sachiez ce que vous faites ou que Vespura vous demande de l'activer.
--------------------------------------------------
    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if freezeTime then
                ...
                TriggerClientEvent('vSync:notify', source, 'Le temps ~y~n\'est plus gelé~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'êtes pas autorisé à utiliser cette commande !')
        end
    else
        freezeTime = not freezeTime
        if freezeTime then
...
RegisterCommand('weather', function(source, args)
    if source == 0 then
        ...
        if isAllowedToChange(source) then
            ...
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Error: ^1Vous n\'avez pas accès à cette commande !')
...
RegisterCommand('blackout', function(source)
    if source == 0 then
        ...
        if blackout then
            print("Blackout est maintenant activé.")
        else
            print("Blackout est maintenant désactivé.")
        end
    else
        if isAllowedToChange(source) then
            blackout = not blackout
            if blackout then
                ...
                TriggerClientEvent('vSync:notify', source, 'Blackout est maintenant ~r~désactivé~s~.')
            end
            TriggerEvent('vSync:requestSync')
        end
    end
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        TriggerClientEvent('vSync:notify', source, 'Temps réglé sur ~y~Matin~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent('vSync:notify', source, 'Temps réglé sur ~y~Midi~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent('vSync:notify', source, 'Temps réglé sur ~y~Soir~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
RegisterCommand('night', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        TriggerClientEvent('vSync:notify', source, 'Temps réglé sur ~y~Nuit~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
function NextWeatherStage()
    ...
    if debugprint then
        ...
        print("[vSync] Réinitialiser la minuterie à 20 minutes.\n")
    end
end
