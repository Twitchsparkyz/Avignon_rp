...
Citizen.CreateThread(function()
    ...
    for i=1, #Config.Locations do
        if Config.Locations[i]["cashier"]["spawn"] == true then
            local cashier = Config.Locations[i]["cashier"]
            if cashier then
...
Citizen.CreateThread(function()
    for i=1, #Config.Locations do
        ...
        local blipSprite = Config.Locations[i]["blipSprite"]
        local blipColor = Config.Locations[i]["blipColor"]

        if blip then
            if not DoesBlipExist(blip["id"]) then
                blip["id"] = AddBlipForCoord(blip["x"], blip["y"], blip["z"])
                SetBlipSprite(blip["id"], blipSprite)
                SetBlipDisplay(blip["id"], 4)
                SetBlipScale(blip["id"], 1.0)
                SetBlipColour(blip["id"], blipColor)
