    ...
    while true do
        ...
        if payAmount > 0 then
            for shop = 1, #Config.Locations do
                ...
                if dist <= 20.0 then
                    if dist >= 12.0 then
                        pNotify("tu as quitté le magasin, ton panier est maintenant vide!", "erreur", 2500)
                        payAmount = 0
                        Basket = {}
                    end
                end
...
CashRegister = function(titel)
        local elements = {
            ...
            {label = 'montant a payer: <span style="color:green">$' .. payAmount ..'</span>'},
        }
                ...
                if data.current.value == "yes" then
                    ...
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'penis2',
                        {
                            ...
                            align    = 'center',
                            elements = {
                                {label = "Payer avec votre cash ", value = "cash"},
                                {label = "Payer avec votre carte banquaire", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    pNotify("vous n'avez pas d'argent!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("vous avez pris " .. data.current.value .. " article " .. data.current["realLabel"] .. " dans votre panier", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'montant a payer: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Tout supprimé de votre panier.", "error", 2500)
                end
