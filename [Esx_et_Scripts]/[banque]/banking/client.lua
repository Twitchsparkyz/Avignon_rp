...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true ) then
    if (IsInVehicle()) then
	  ...
	  exports.JD_logs:discord("Depot argent €: "..amount, GetPlayerServerId(PlayerId()), 0, '#32FF00', 'Depot_argent')
	  TriggerServerEvent('bank:balance')
	...
	if depositAtATM then
	  TriggerServerEvent("bank:deposit", tonumber(amount))
	  exports.JD_logs:discord("Depot argent 2: "..amount, GetPlayerServerId(PlayerId()), 0, '#32FF00', 'Depot_argent')
	  TriggerServerEvent('bank:balance')
	  TriggerEvent('banking:viewBalance')
	  TriggerEvent('banking:viewCash')
	else
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true) then
    if (IsInVehicle()) then
	  ...
	  TriggerEvent('banking:viewBalance')
	  TriggerEvent('banking:viewCash')
	  exports.JD_logs:discord("Retrait argent €:  "..amount, GetPlayerServerId(PlayerId()), 0, '#FF0000', 'retrait_argent')
    end
  end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if (playing ~= false) then
	...
	exports.JD_logs:discord("transfert argent €: "..amount, GetPlayerServerId(PlayerId()), 0, '#FF7400', 'transfer_argent')
		  else
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1This player is not online!");
  end
  TriggerServerEvent('bank:balance')
