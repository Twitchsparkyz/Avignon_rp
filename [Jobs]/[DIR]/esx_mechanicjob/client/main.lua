	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					{label = _U('flat_bed'),  value = 'mercodep'},
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		align    = 'top-left',
		elements = {
		    {label = '_____ Actions Véhicule _____',     value = ''},
			...
			{label = 'Mettre / Retirer du Plateau',      value = 'dep_vehicle'},
			{label = '_____ Autre _____',                value = ''},
			{label = 'Placer Objet',                     value = 'object_spawner'},
		...
		elseif data.current.value == 'dep_vehicle' then
			...
			local towmodel = GetHashKey('mercodep')
	...
	while true do
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						if
							GetEntityModel(vehicle) == GetHashKey('mercodep')   or
		...
		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
				...
				if IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), GetHashKey('mercodep')) then
					StartNPCJob()
				else
