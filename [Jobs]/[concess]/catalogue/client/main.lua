...
AddEventHandler('esx_qalle_bilpriser:hasEnteredMarker', function (zone)
  if zone == 'Katalog' then
    ...
    CurrentActionMsg  = 'Appuyer sur ~b~E~w~ pour ouvrir le menu'
    CurrentActionData = {}
  end

  if zone == 'GoDownFrom' then
    CurrentAction     = 'go_down_from'
    CurrentActionMsg  = 'Appuyer sur ~b~E~w~ pour entrer dans le catalogue'
    CurrentActionData = {}
  end

  if zone == 'GoUpFrom' then
    CurrentAction     = 'go_up_from'
    CurrentActionMsg  = 'Appuyer sur ~b~E~w~ pour sortir du catalogue'
    CurrentActionData = {}
  end
    ...
    function (data, menu)
      ...
      ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'shop_confirm',
        {
          ...
          align = 'top-left',
          elements = {
            {label = '' .. vehicleData.name .. _U('costs') .. '<span style="color:orange;">Demander au vendeur', value = 'yes'},
            {label = _U('back'), value = 'no'},
          },
        },
        function (data2, menu2)
          if data2.current.value == 'yes' then
             sendNotification(_U('contact_dealer') .. '<span style="color:orange;">Demander au vendeur', 'warning', 5000)
          end
