...
AddEventHandler("AppelemsTropBien", function(coords, id)
	...
	AppelCoords = coords
	AppelID = id
	ESX.ShowAdvancedNotification("Samu / Pompier", "~b~Demande de Médecin", "Quelqu'un à besoin d'un Médecin ! Ouvrez votre menu [F6] pour intervenir", "CHAR_CALL911", 8)
end)
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'main'), true, true, true, function()
					RageUI.Checkbox("Prendre/Quitter son service", nil, ambuservice, { Style = RageUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
						...
						if ambuservice then
							...
							RageUI.Separator("~s~↓ ~b~Annonces Samu / Pompier ~s~↓")
...
Citizen.CreateThread(function()
    while true do
       ...
       RageUI.IsVisible(RMenu:Get('ambulance', 'gestr'), true, true, true, function()
        RageUI.Separator("~y~Type~s~ : Demande de Médecin")
        ...
        RageUI.CenterButton("~r~Refuser ~s~l'appel~s~ ~b~N°".. supprimer, nil, {}, true, function(Hovered, Active, Selected)
			if (Selected) then
				pris = false
                ESX.ShowAdvancedNotification("ems", "~b~Demande de Médecin", "Vous avez refuser l'appel.", "CHAR_CALL911", 8)
                TriggerServerEvent('ems:supprimeappels', supprimer)
            end
        end)
       end, function()
...
function openPharma()
	...
	Citizen.CreateThread(function()
		...
		AddEventHandler("emsAppelPris", function(Xid, XAppelCoords)
			ESX.ShowAdvancedNotification("Samu / Pompier", "~b~Demande de Médecin", "Vous avez pris l'appel, ~y~Dirigez-vous là-bas ~s~via votre GPS !", "CHAR_CALL911", 2)   
		...
		function afficherTextVolant(XAcoords, XAid)
			...
			AddTextComponentSubstringPlayerName("~b~[Samu / Pompier] Appel en cours")
			EndTextCommandSetBlipName(emsBlip)
			 ...
			 if GetDistanceBetweenCoords(XAcoords, GetEntityCoords(GetPlayerPed(-1))) < 10.0 then
				ESX.ShowAdvancedNotification("EMS", "~b~GPS du Samu", "Vous êtes arrivé !", "CHAR_CALL911", 2)   
				TriggerEvent("EMS:ClearAppel")
		end
		Wait(1)
		end
		...
		Citizen.CreateThread( function()	
			while true do
				...
				if enService then
					...
					DrawAdvancedText(0.988, 0.236, 0.005, 0.0028, 0.4, "~b~"..NomAppel.." ~w~à pris le dernier appel", 255, 255, 255, 255, 6, 0)
