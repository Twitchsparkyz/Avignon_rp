...
AddEventHandler('esx_ambulancejob:revive', function(target)
	...
	exports.JD_logs:discord("Revive EMS", _source, target, '#F1ED04', 'Revive_ambulancejob')
end)
...
AddEventHandler('esx:onPlayerSpawn', function()
	if deadPlayers[source] then
		...
		TriggerClientEvent('esx_ambulancejob:setDeadPlayers', -1, deadPlayers)
		exports.JD_logs:discord("Revive Auto", source, 0, '#FF0000', 'Revive_auto')
	end
end)
...
if Config.EarlyRespawnFine then
	...
	AddEventHandler('esx_ambulancejob:payFine', function()
		...
		exports.JD_logs:discord("Revive early, a payé : € "..fineAmount, source, 0, '#8204F1', 'Revive_early')
	end)
end
...
TriggerEvent('es:addGroupCommand', 'revive', 'mod', function(source, args, user)
	if args[1] ~= nil then
		if GetPlayerName(tonumber(args[1])) ~= nil then
			...
			exports.JD_logs:discord("Admin revive"..tonumber(args[1]), source, tonumber(args[1]), '#001FFF', 'Revive_admin')
		end
	else
		TriggerClientEvent('esx_ambulancejob:revive', source)
		exports.JD_logs:discord("Admin revive solo", source, 0, '#59FF00', 'Revive_admin')
	end
...
AddEventHandler('EMS:Ouvert', function()
    ...
    for i=1, #xPlayers, 1 do
        ...
        TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Samu / Pompier', '~b~Annonce Samu', 'Un ~g~Médecin~s~ est en Service ! Votre santé avant tout !', 'CHAR_CALL911', 7)
    end
end)
...
AddEventHandler('EMS:Fermer', function()
    ...
    for i=1, #xPlayers, 1 do
        local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
        TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Samu / Pompier', '~b~Annonce Samu', 'Il n\'y a plus ~r~de Médecin~s~ en Service ! ', 'CHAR_CALL911', 7)
    end
end)
