...
TriggerEvent('esx_phone:registerNumber', 'Gang', 'alerte Gang', true, true)
...
ESX.RegisterServerCallback('esx_Gangjob:getVehicleFromPlate', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
...
RegisterServerEvent('esx_Gangjob:prendreitems')
AddEventHandler('esx_Gangjob:prendreitems', function(itemName, count)
	local _source = source
...
RegisterNetEvent('esx_Gangjob:stockitem')
AddEventHandler('esx_Gangjob:stockitem', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	local sourceItemCount = xPlayer.getInventoryItem(itemName).count
		
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_Gang', function(inventory)
		local inventoryItem = inventory.getItem(itemName)
		
		if  sourceItemCount >= count and count > 0 then
...
ESX.RegisterServerCallback('esx_Gangjob:inventairejoueur', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_Gangjob:prendreitem', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_Gang', function(inventory)
		cb(inventory.items)
	end)
end)
...
RegisterServerEvent('esx_Gangjob:message')
AddEventHandler('esx_Gangjob:message', function(target, msg)
...
ESX.RegisterServerCallback('esx_Gangjob:getBlackMoneySociety', function(source, cb)
  local _source = source
  local xPlayer    = ESX.GetPlayerFromId(_source)
  local blackMoney = 0
  local items      = {}
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_Gang_black', function(account)
...
RegisterServerEvent('esx_Gangjob:getItem')
AddEventHandler('esx_Gangjob:getItem', function(type, item, count)
...
RegisterServerEvent('esx_Gangjob:putItem')
AddEventHandler('esx_Gangjob:putItem', function(type, item, count)
  ...
  if type == 'item_weapon' then
    TriggerEvent('esx_datastore:getSharedDataStore', 'society_Gang_weapons', function(store)
      local storeWeapons = store.get('weapons')
...
ESX.RegisterServerCallback('esx_Gangjob:getPlayerInventory2', function(source, cb)
  local _source = source
  local xPlayer    = ESX.GetPlayerFromId(_source)
