...
function refreshPlayerWhitelisted()
	...
	for k,v in ipairs(Config.WhitelistedCops) do
		if (v == ESX.PlayerData.job.name) or (v == ESX.PlayerData.job2.name) then
			return true
		end
	end
...
--pour crochetage
		local playerPed = PlayerPedId()
		local playerCoords = GetEntityCoords(playerPed)
					-- crochetage vehicules
		elseif IsAnyVehicleNearPoint(playerCoords.x, playerCoords.y, playerCoords.z, 3.0) and IsPedUsingScenario(playerPed, "WORLD_HUMAN_WELDING")== true and Config.CarJackingAlert then
